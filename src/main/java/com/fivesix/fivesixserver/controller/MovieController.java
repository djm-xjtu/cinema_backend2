...
public class MovieController {
    ...
    public Object listByCategory(@PathVariable("cid") int cid, @PathVariable("dateOrRate") int dateOrRate) throws Exception {
        ...
        if(dateOrRate == 1){
            return res.stream().sorted(Comparator.comparingDouble(Movie::getRate).reversed()).collect(Collectors.toList());
        }
        else{
            return res.stream().sorted(Comparator.comparing(Movie::getDate).reversed()).collect(Collectors.toList());
        }
    }
